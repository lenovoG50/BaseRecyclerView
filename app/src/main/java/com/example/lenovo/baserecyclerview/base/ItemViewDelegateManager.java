...
public class ItemViewDelegateManager<T> {
    SparseArrayCompat<T> delegates = new SparseArrayCompat<>();
    ...
    public ItemViewDelegateManager<T> addDelegate(ItemViewDelegate<T> delegate) {
        ...
        if (delegate != null) {
            delegates.put(viewType, (T) delegate);
            viewType++;
        }
        return this;
    }
    ...
    public ItemViewDelegate<T> removeDelegate(ItemViewDelegate<T> delegate) {
        ...
        int indexToRemove = delegates.indexOfValue((T) delegate);
        if (indexToRemove >= 0) {
    ...
    public int getItemViewType(ItemViewDelegate itemViewDelegate) {
        return delegates.indexOfValue((T) itemViewDelegate);
    }
}